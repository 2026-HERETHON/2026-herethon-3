...
function refreshReviewSection(legalDongId, legalDongName) {
  ...
  return fetch(reviewPageUrl)
    ...
    .then((htmlText) => {
      ...
      // 🎯 list.html이 이제 #rating-summary에 data-night/data-amenity/data-mood를
      // 직접 내려주므로, 텍스트를 정규식으로 긁어낼 필요 없이 바로 읽으면 된다.
      ...
      // =====================================================================
      // 🎯 [★ 후기 카드 리스트도 같은 응답에서 같이 가져오기]
      // list.html의 #rightSB-reviewCardContainer 안에 .review-data-item으로
      // 후기마다 data-* 속성이 이미 구조화되어 내려오므로, 텍스트를 파싱하지 않고
      // dataset을 그대로 읽으면 된다.
      // =====================================================================
      const reviewDivs = Array.from(
        doc.querySelectorAll("#rightSB-reviewCardContainer .review-data-item"),
      );

      const parsedReviews = reviewDivs.map((div) => ({
        id: div.dataset.reviewId,
        liked: div.dataset.liked === "true", // 로그인 유저가 이미 누른 좋아요인지
        nickname: div.dataset.nickname || "익명",
        residence: div.dataset.residence || `${legalDongName} 거주 중`,
        date: div.dataset.date || "",
        score: parseFloat(div.dataset.score) || 0,
        content: div.dataset.content || "",
        likes: parseInt(div.dataset.likes, 10) || 0,
      }));

      console.log(
        `🔥 list.html에서 받은 진짜 후기 ${parsedReviews.length}건:`,
        parsedReviews,
      );
...
function openQuestionDetail(questionId) {
  ...
  fetch(`http://127.0.0.1:8000/qna/question/${questionId}/`)
    ...
    .then((htmlText) => {
      ...
      // 🎯 detail.html의 <body data-question="..."> 에 질문 내용이 이미
      // data-* 속성으로 내려오므로 바로 읽으면 된다.
      const questionText = doc.body?.dataset.question || "";
      if (titleEl) titleEl.textContent = questionText;

      // 🎯 답변도 .qna-answer-item마다 data-nickname/data-content로 내려오므로
      // "strong+p를 가진 div"를 구조로 추측할 필요 없이 dataset을 바로 읽는다.
      const answerDivs = Array.from(
        doc.querySelectorAll("#qna-answer-list .qna-answer-item"),
      );
...
function refreshQnaSection(legalDongId) {
  ...
  return fetch(qnaPageUrl)
    ...
    .then((htmlText) => {
      ...
      // 🎯 qna/list.html의 #rightSB-qnaCardContainer 안에 .qna-data-item으로
      // 질문마다 data-* 속성이 이미 구조화되어 내려오므로, href/텍스트를
      // 파싱하지 않고 dataset을 그대로 읽으면 된다.
      const questionDivs = Array.from(
        doc.querySelectorAll("#rightSB-qnaCardContainer .qna-data-item"),
      );

      const parsedQnas = questionDivs.map((div) => ({
        id: div.dataset.questionId,
        question: div.dataset.question || "",
        answerCount: parseInt(div.dataset.answerCount, 10) || 0,
      }));

      console.log(
        `🔥 qna/list.html에서 받은 진짜 질문 ${parsedQnas.length}건:`,
        parsedQnas,
      );
